...
```{r}
ord.Model <- polr(Quality ~ assess_percentile_na_proportion + assess_col_na + extract_na +
...
table(test$num.predictions, test$num.Quality)
```
...
```{r}
# Top is true value, left is predicted
table(round(as.numeric(test$num.predictions,0)), test$num.Quality)
```
